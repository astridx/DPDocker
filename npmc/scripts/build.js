...
function buildAssets(root, assets, includeVendor)
{
	...
	// Loop over the phpvendor assets
	console.log('Started to build the phpvendor assets for ' + root);
	assets.phpvendor.forEach(asset => {
		// Make sure that the destination folder exists
		let dir = path.dirname(root + '/' + asset.dest);
		if (!fs.existsSync(dir)) {
			fs.mkdirSync(dir, {recursive: true});
		...
		asset.src.forEach((source, index) => {
			// Determine file name
			let file = root + '/package/vendor/' + source;
			if (!fs.existsSync(file)) {
				file = root + '/' + source;
			}
